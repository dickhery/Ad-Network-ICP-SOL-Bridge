...
thread_local! {
    ...
    static SERVICE_NONCE: RefCell<u64> = RefCell::new(0);
}
...
fn require_owner(sol_pubkey: &str) -> Result<(), String> {
    ...
    match owner {
        ...
        Some(_) => Err("Unauthorized: wallet linked to a different Internet Identity".to_string()),
        None => Err("Unauthorized: link this Solana wallet to your Internet Identity first".to_string())
    }
}
...
#[update]
async fn get_adnet_sol_address() -> String {
    let service_pk = get_user_sol_pk_for_path("adnet_service".as_bytes().to_vec()).await;
    bs58::encode(service_pk).into_string()
}
...
#[update]
async fn transfer_sol_from_service(to: String, amount: u64) -> String {
    let slot = match sol_get_finalized_slot().await {
        Ok(s) => s,
        Err(e) => return format!("Failed to get slot: {}", e),
    };
    ...
    let sign_args = SignWithSchnorrArgs {
        message: msg_ser.clone(),
        derivation_path: vec!["adnet_service".as_bytes().to_vec()],
        key_id: KEY_ID.clone(),
        aux: None,
    };
...
#[query]
fn verify_signature_pub(sol_pubkey: String, message: String, signature: Vec<u8>) -> bool {
    verify_signature(&sol_pubkey, message.as_bytes(), &signature)
}

#[update]
fn verify_and_bump(sol_pubkey: String, message: String, signature: Vec<u8>, nonce: u64) -> bool {
    let current = read_or_init_nonce(&sol_pubkey);
    if nonce != current { return false; }
    if !verify_signature(&sol_pubkey, message.as_bytes(), &signature) { return false; }
    NONCE_MAP.with(|m| m.borrow_mut().insert(sol_pubkey, current + 1));
    true
}
